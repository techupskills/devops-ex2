...
def get_crumb():
    try:
        r = requests.get(f"{JENKINS_URL}/crumbIssuer/api/json",
                         auth=HTTPBasicAuth(USER, TOKEN), timeout=15)
...
def main():
    if not TOKEN:
        print("JENKINS_TOKEN is required (user API token).", file=sys.stderr)
        sys.exit(2)
    ...
    r = requests.post(f"{JENKINS_URL}/createItem?name={JOB_NAME}",
                      data=config_xml, headers=headers,
                      auth=HTTPBasicAuth(USER, TOKEN), timeout=30)

    if r.status_code in (200, 201):
        print(f"Job '{JOB_NAME}' created or already exists.")
        sys.exit(0)
    if r.status_code == 400 and "already exists" in r.text.lower():
        print(f"Job '{JOB_NAME}' already exists.")
        sys.exit(0)

    print(f"Failed to create job: HTTP {r.status_code} {r.text}", file=sys.stderr)
    sys.exit(1)
